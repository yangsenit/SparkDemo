...
//    //准备数据
    ...
    val topic =Set[String]("my-topic")
    val lines= KafkaUtils.createDirectStream[String,String,StringDecoder, StringDecoder](ssc,kafkaParameters,topic).map(_._2)
    case class schema(data: String,timeStamp: String,userID: String,pageID: String,channelID: String,action: String)
    val sqlContext=new SQLContext(ssc.sparkContext)
    lines.foreachRDD(rdd=>{
      import sqlContext.implicits._
      rdd.map(x=>x.split("\t")).map(x=>{
        schema(x(0),x(1),x(2),x(3),x(4),x(5))
